...
public class UserAmount {
    ...
    public boolean checkDeductableFromLedger(Ledger ledger) {
        LinkedList<Boolean> canBeRemove = new LinkedList<Boolean>();
        TreeMap<String, Integer> ledgerAmountBase = ledger.userAmountBase;
        for (String userAmountKey : this.getUsers()) {
            for (String ledgerKey : ledger.getUsers()) {
                if (ledgerKey.equals(userAmountKey)) {
                    if (ledger.getBalance(ledgerKey) >= this.getBalance(userAmountKey)) {
                        canBeRemove.add(true);
                    } else canBeRemove.add(false);
                }
            }
        }
        if (canBeRemove.contains(false)) return false;
        else return true;
    };
