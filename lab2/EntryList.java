...
public class EntryList{
    ...
    public boolean checkDeductableFromLedger(Ledger ledger){
        UserAmount amt = new UserAmount(this);
        return amt.checkDeductableFromLedger(ledger);
    };
    ...
    public void subtractFromLedger(Ledger ledger) {
        UserAmount amt = new UserAmount(this);
        if (amt.checkDeductableFromLedger(ledger)) {
            for (String userKey : amt.getUsers()) {
                for (String ledgerKey : ledger.getUsers()) {
                    if (ledgerKey.equals(userKey)) {
                        int newUserBalance = ledger.getBalance(ledgerKey) - amt.getBalance(userKey);
                        ledger.setBalance(ledgerKey, newUserBalance);
                    }
                }
            }
        }
